...
from ignite.contrib.handlers import wandb_logger as wandb_logger
...
class BatchGenerator:
    def __init__(self, buffer: ptan.experience.ExperienceReplayBuffer, exp_queue: mp.Queue, fps_handler: ptan_ignite.EpisodeFPSHandler, initial: int, batch_size: int):
        self.buffer = buffer
...
if __name__ == "__main__":
    ...
    wb = wandb_logger.WandBLogger(project="simulation_ne_rl", name=params.run_name, config=params)
    RunningAverage(output_transform=lambda v: v['loss']).attach(engine, "avg_loss")

    episode_handler = wandb_logger.OutputHandler(tag="episodes", metric_names=['reward', 'steps', 'avg_reward'])
    wb.attach(engine, log_handler=episode_handler, event_name=ptan_ignite.EpisodeEvents.EPISODE_COMPLETED)

    # write to tensorboard every 100 iterations
    ptan_ignite.PeriodicEvents().attach(engine)
    handler = wandb_logger.OutputHandler(tag="train", metric_names=['avg_loss', 'avg_fps'],
                                      output_transform=lambda a: a)
    wb.attach(engine, log_handler=handler, event_name=ptan_ignite.PeriodEvents.ITERS_100_COMPLETED)

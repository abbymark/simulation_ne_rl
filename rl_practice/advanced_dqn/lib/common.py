...
def setup_ignite(engine: Engine, params: SimpleNamespace, exp_source, run_name:str, extra_metrics: Iterable[str] = ()):
    ...
    def episode_completed(trainer:Engine):
        ...
        print(f"Episode {trainer.state.episode}: reward={trainer.state.episode_reward}, steps={trainer.state.episode_steps}, speed={trainer.state.metrics.get('avg_fps', 0):.2f} f/s, elapsed={timedelta(seconds=int(passed))}")

    @engine.on(ptan_ignite.EpisodeEvents.BOUND_REWARD_REACHED)
    def game_solved(trainer:Engine):
        passed = trainer.state.metrics['time_passed']
        print(f"Game solved in {time_delta(seconds=int(passed))}, after {trainer.state.episode} episodes and {trainer.state.iteration} iterations!")
